...
describe('Default Model Configuration', () => {
  ...
  test('should use gpt-5.6-terra as default model when no model specified', async () => {
  ...
  test('should include default model in response metadata', async () => {
    ...
    expect(result.content[0]._meta?.model).toBe('gpt-5.6-terra');
    expect(result.structuredContent?.model).toBe('gpt-5.6-terra');
